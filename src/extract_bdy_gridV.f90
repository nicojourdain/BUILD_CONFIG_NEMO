...
&                                       myb, mxbv, glamv_ID, gphiv_ID, e1v_ID, e2v_ID,               &
...
CHARACTER(LEN=150)                   :: file_coord, file_in_gridV, file_bdy_gridV2d, file_mesh_mask, &
...
REAL*4,ALLOCATABLE,DIMENSION(:,:)    :: glamv, gphiv, e1v, e2v, nav_lon, nav_lat, nav_lon_bdy, nav_lat_bdy
...
!- Read e3v and e3v in large-scale/global file:
    ...
    DO kday=1,31
      ...
      IF ( existfile ) THEN
        ...
        do kbdy=1,mxbv
          ...
          do kt=1,mtime
          do kz=1,mdepthv
            vomecrty_bdy(kbdy,1,kz,kt) = vomecrty( iGLO, jGLO, kz, kt )
            vobtcrty_bdy(kbdy,1,   kt) = vobtcrty( iGLO, jGLO,     kt )
          enddo
          enddo
        enddo

        write(*,*) '@@4 ', minval(vomecrty_bdy)
