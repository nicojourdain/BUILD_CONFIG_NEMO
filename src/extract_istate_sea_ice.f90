...
&          mx_CHI, my_CHI, mz_CHI, tmask_CHI_ID, fidSAL, fidTEMP, siconc_ID, sithic_ID, snthic_ID,       &
...
status = NF90_OPEN(TRIM(file_in_T),0,fidTin); call erreur(status,.TRUE.,"Read PARENT sea ice file")

ALLOCATE(  siconc_PAR(mx_PAR,my_PAR,mz_PAR)  )   
ALLOCATE(  sithic_PAR(mx_PAR,my_PAR,mz_PAR)  )   
ALLOCATE(  snthic_PAR(mx_PAR,my_PAR,mz_PAR)  )   
...
ALLOCATE( siconc_CHI(mx_CHI,my_CHI,mz_CHI) )
ALLOCATE( sithic_CHI(mx_CHI,my_CHI,mz_CHI) )
ALLOCATE( snthic_CHI(mx_CHI,my_CHI,mz_CHI) )
...
ALLOCATE( tmp_siconc_CHI(mx_CHI,my_CHI,mz_CHI) )
ALLOCATE( tmp_sithic_CHI(mx_CHI,my_CHI,mz_CHI) )
ALLOCATE( tmp_snthic_CHI(mx_CHI,my_CHI,mz_CHI) )
missing(:,:,:)=0
siconc_CHI(:,:,:)=0.d0
sithic_CHI(:,:,:)=0.d0
snthic_CHI(:,:,:)=0.d0
   ...
   if ( iPAR .ge. 1 .and. jPAR .ge. 1 ) then
     do kk=1,mz_CHI
       if ( tmask_PAR(iPAR,jPAR,kk) .eq. 1 ) then
         siconc_CHI(iCHI,jCHI,kk) = siconc_PAR(iPAR,jPAR,kk) * tmask_CHI(iCHI,jCHI,kk) 
         sithic_CHI(iCHI,jCHI,kk) = sithic_PAR(iPAR,jPAR,kk) * tmask_CHI(iCHI,jCHI,kk) 
         snthic_CHI(iCHI,jCHI,kk) = snthic_PAR(iPAR,jPAR,kk) * tmask_CHI(iCHI,jCHI,kk) 
  ...
  do kk=1,mz_CHI
    if ( missing(iCHI,jCHI,kk) .eq. 1 ) then
      ...
      if (iout) then
        ...
        tmp_siconc_CHI(iCHI,jCHI,kk) = siconc_CHI(iii,jjj,kkk)
        tmp_sithic_CHI(iCHI,jCHI,kk) = sithic_CHI(iii,jjj,kkk)
        tmp_snthic_CHI(iCHI,jCHI,kk) = snthic_CHI(iii,jjj,kkk)
        exit
      elseif ( rz .eq. nn_rzmax .and. kiter .eq. nn_iter ) then
        ...
        tmp_missing(iCHI,jCHI,kk) = 0
        tmp_siconc_CHI(iCHI,jCHI,kk) = rn_temp
        tmp_sithic_CHI(iCHI,jCHI,kk) = rn_sal
        tmp_snthic_CHI(iCHI,jCHI,kk) = rn_temp
        exit
      endif
      enddo !-rz
...
if ( nn_smooth .gt. 1 ) then
  ...
  do kk=1,mz_CHI
    ...
    if ( tmask_CHI(iCHI,jCHI,kk) .eq. 1 ) then 
      tmp_siconc_CHI(iCHI,jCHI,kk) =   SUM( SUM( siconc_CHI(im1:ip1,jm1:jp1,kk) * tmask_CHI(im1:ip1,jm1:jp1,kk), 2), 1) &
      &                              / SUM( SUM(                           1.0  * tmask_CHI(im1:ip1,jm1:jp1,kk), 2), 1)
      tmp_sithic_CHI(iCHI,jCHI,kk) =   SUM( SUM( sithic_CHI(im1:ip1,jm1:jp1,kk) * tmask_CHI(im1:ip1,jm1:jp1,kk), 2), 1) &
      &                              / SUM( SUM(                           1.0  * tmask_CHI(im1:ip1,jm1:jp1,kk), 2), 1)
      tmp_snthic_CHI(iCHI,jCHI,kk) =   SUM( SUM( snthic_CHI(im1:ip1,jm1:jp1,kk) * tmask_CHI(im1:ip1,jm1:jp1,kk), 2), 1) &
      &                              / SUM( SUM(                           1.0  * tmask_CHI(im1:ip1,jm1:jp1,kk), 2), 1)
    else
      tmp_siconc_CHI(iCHI,jCHI,kk) = 0.d0
      tmp_sithic_CHI(iCHI,jCHI,kk) = 0.d0
      tmp_snthic_CHI(iCHI,jCHI,kk) = 0.d0
    endif
  enddo
  enddo
  enddo
...
status = NF90_DEF_DIM(fidTEMP,"z",mz_CHI,dimID_z)                               ; call erreur(status,.TRUE.,"def_dimID_deptht")
...
status = NF90_DEF_VAR(fidTEMP,"siconc",NF90_FLOAT,(/dimID_x,dimID_y,dimID_z,dimID_time_counter/),siconc_ID)
call erreur(status,.TRUE.,"def_var_siconc_ID")
status = NF90_DEF_VAR(fidTEMP,"sithic",NF90_FLOAT,(/dimID_x,dimID_y,dimID_z,dimID_time_counter/),sithic_ID)
call erreur(status,.TRUE.,"def_var_sithic_ID")
status = NF90_DEF_VAR(fidTEMP,"snthic",NF90_FLOAT,(/dimID_x,dimID_y,dimID_z,dimID_time_counter/),snthic_ID)
call erreur(status,.TRUE.,"def_var_snthic_ID")

status = NF90_PUT_ATT(fidTEMP,siconc_ID,"associate","time_counter, z, y, x") ; call erreur(status,.TRUE.,"put_att_siconc_ID")
...
status = NF90_PUT_ATT(fidTEMP,sithic_ID,"associate","time_counter, z, y, x") ; call erreur(status,.TRUE.,"put_att_sithic_ID")
...
status = NF90_PUT_ATT(fidTEMP,snthic_ID,"associate","time_counter, z, y, x") ; call erreur(status,.TRUE.,"put_att_snthic_ID")
