...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! Used to build netcdf gridT file for BDY
!
! 0- Initialiartions
  ...
  DO kmonth=1,12
    ...
    DO kday=1,31
      ...
      IF ( existfile ) THEN
        ...
        status = NF90_OPEN(TRIM(file_in_gridT),0,fidT)                ; call erreur(status,.TRUE.,"read EXT TS") 
        ...
        call erreur(status,.TRUE.,"None existing temperature, or unknown variable name")
        ...
        status = NF90_OPEN(TRIM(file_in_gridS),0,fidS)                ; call erreur(status,.TRUE.,"read EXT TS") 
        ...
        call erreur(status,.TRUE.,"None existing salinity, or unknown variable name")
        ...
        do kbdy=1,mxbt
          ...
          do kt=1,mtime
          do kz=1,mdeptht
            votemper_bdy(kbdy,1,kz,kt) = votemper( iPAR, jPAR, kz, kt )
            vosaline_bdy(kbdy,1,kz,kt) = vosaline( iPAR, jPAR, kz, kt )
          enddo
          enddo
        enddo
