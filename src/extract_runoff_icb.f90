...
&                                       nfmt, lon_ID, lat_ID, iREG, jREG, tmask_GLO_ID, ntest, ntest2, mz_GLO, &
...
REAL(KIND=8),ALLOCATABLE,DIMENSION(:,:,:)    :: runoff_GLO, runoff_REG, tmp_runoff_REG, icb_melt_GLO, icb_melt_REG, tmp_icb_melt_REG
    ...
    DO kday=1,31
      ...
      IF ( existfile ) THEN
        ...
        if ( status .ne. 0 ) then
          ...
          status = NF90_GET_VAR(fidRNF,icb_melt_ID,icb_melt_GLO) ; call erreur(status,.TRUE.,"getvar_icb_melt")
        endif
        ...
        ALLOCATE( runoff_REG(mx_REG,my_REG,mtime), icb_melt_REG(mx_REG,my_REG,mtime) )
        ALLOCATE( tmp_runoff_REG(mx_REG,my_REG,mtime), tmp_icb_melt_REG(mx_REG,my_REG,mtime) )
        missing(:,:)=0
        runoff_REG(:,:,:)=0.d0
        icb_melt_REG(:,:,:)=0.d0
          ...
          if ( iGLO .ge. 1 .and. jGLO .ge. 1 ) then
            if ( tmask_GLO(iGLO,jGLO) .eq. 1 ) then
              do l=1,mtime
                ...
                icb_melt_REG(iREG,jREG,l) = icb_melt_GLO(iGLO,jGLO,l) * tmask_REG(iREG,jREG)
              enddo
            else
              runoff_REG(iREG,jREG,:) = 0.e0 
              icb_melt_REG(iREG,jREG,:) = 0.e0 
            endif
          else
            runoff_REG(iREG,jREG,:) = 0.e0
            icb_melt_REG(iREG,jREG,:) = 0.e0
        ...
        do jREG=1,my_REG
            ...
            if ( tmask_REG(iREG,jREG) .eq. 1 ) then 
              do l=1,mtime
                ...
                tmp_icb_melt_REG(iREG,jREG,l) =   SUM( SUM( icb_melt_REG(im1:ip1,jm1:jp1,l) * tmask_REG(im1:ip1,jm1:jp1), 2), 1) &
                &                               / SUM( SUM(                            1.0  * tmask_REG(im1:ip1,jm1:jp1), 2), 1)
              enddo
            else
              tmp_runoff_REG(iREG,jREG,:) = 0.e0
              tmp_icb_melt_REG(iREG,jREG,:) = 0.e0
            endif
        enddo
        enddo
        runoff_REG(:,:,:)=tmp_runoff_REG(:,:,:)
        icb_melt_REG(:,:,:)=tmp_icb_melt_REG(:,:,:)
        ...
        status = NF90_PUT_VAR(fidM,icb_melt_ID,icb_melt_REG) ; call erreur(status,.TRUE.,"var_icb_melt_ID")
        ...
        !--       
        DEALLOCATE( runoff_GLO, icb_melt_GLO, time )
        DEALLOCATE( runoff_REG, icb_melt_REG, socoefr )
        DEALLOCATE( tmp_runoff_REG, tmp_icb_melt_REG )
